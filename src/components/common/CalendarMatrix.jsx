...
export default function CalendarMatrix({ title = "Monthly Schedule / Attendance Matrix", subtitle = "September, 2026" }) {
  const days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
  
  // Sample 5 weeks of September 2026
  const calendarDays = [
    { num: 31, status: 'prev' },
    ...
    <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-xs flex flex-col justify-between">
      ...
      <div className="grid grid-cols-7 gap-1.5 text-center">
        {calendarDays.map((d, i) => {
          ...
          if (d.status === 'draft') cellStyle = "bg-blue-50 border-blue-200 text-blue-800";
          if (d.status === 'prev') cellStyle = "bg-slate-50 border-transparent text-slate-300";

          return (
            <div 
              key={i} 
              className={`p-2 rounded-xl border text-xs flex flex-col items-center justify-center transition-transform hover:scale-105 cursor-pointer ${cellStyle}`}
            >
      ...
      <div className="mt-4 pt-3 border-t border-slate-100 flex flex-wrap items-center gap-3 text-[10px] font-bold text-slate-600">
        <span className="flex items-center"><span className="w-2 h-2 rounded-full bg-amber-400 mr-1" /> Submitted</span>
        <span className="flex items-center"><span className="w-2 h-2 rounded-full bg-emerald-500 mr-1" /> Approved</span>
        <span className="flex items-center"><span className="w-2 h-2 rounded-full bg-blue-400 mr-1" /> Draft</span>
        <span className="flex items-center"><span className="w-2 h-2 rounded-full bg-slate-300 mr-1" /> Not Filled</span>
        <span className="flex items-center"><span className="w-2 h-2 rounded-full bg-slate-200 mr-1" /> Weekend</span>
      </div>
    </div>
  );
}
